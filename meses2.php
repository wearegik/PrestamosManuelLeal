       ...
       $datos[]=$row;
    		...
    		foreach ($datos as $key => $value)			
    		{
    		   ...
    		   $rowsumaabono = mysql_fetch_array($sqlsumaabono);
                ...
                $consultatotal = "SELECT concat(`c`.`Nombres`,' ',`c`.`Apellidos`) AS `Nombres`,sum(`p`.`Valor`) AS `prestamo`,`p`.`Cuotas` AS `Cuotas`,(case when (`a`.`valor` <> 0) then sum(`a`.`valor`) else 0 end) AS `totalabonos`,(`p`.`Valor` - (case when (`a`.`valor` <> 0) then sum(`a`.`valor`) else 0 end)) AS `restante` from ((`prestamo` `p` join `cliente` `c` on((`p`.`idcliente` = `c`.`idcliente`))) left join `abonos` `a` on((`p`.`idprestamo` = `a`.`idprestamo`))) where (`p`.`estado` = 0) and p.idprestamo = ".$value['idprestamo']." group by `p`.`idcliente`,`p`.`idprestamo`,`p`.`Cuotas`" ;
    		    $sqltotal = mysql_query($consultatotal, $conexion);
    		    $rowtotal = mysql_fetch_array($sqltotal);

    			echo '<ul class="tabla">';

    			echo '<li>'.$value['Nombres'].$value['Apellidos'].'</li>';
    			echo '<li>'.number_format($value['valorprestamo']).'</li>';
				//echo '<li>'.$nuevafecha.'</li>';
				//echo '<li>'.date("Y-m-j",strtotime($value['fechaprestamo'])).'</li>';
    			echo '<li>'.$value['tiempoprestamo'].'</li>';
    			echo '<li>'.number_format($rowtotal['totalabonos']).'</li>';
    			...
    			<?php
    			echo '<li>'.$pagint.'</li>';
    			echo '<li>'.number_format($rowtotal['restante']).'</li>'; 
